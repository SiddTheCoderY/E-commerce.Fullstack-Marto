...
export const toggleProductToCart = asyncHandler(async (req, res) => {
  ...
  const remainingProductInCart = await cart.populate("items.product");
...
export const getCartProducts = asyncHandler(async (req, res) => {
  ...
  if (!cart || cart.items.length === 0) {
    return res
      .status(200)
      .json(new ApiResponse(200, [], "No product inside cart yet"));
  }
...
export const updateCartProductCount = asyncHandler(async (req, res) => {
  ...
  res
    .status(200)
    .json(
      new ApiResponse(200, sortedItems, "Product quantity updated successfully")
    );
});
